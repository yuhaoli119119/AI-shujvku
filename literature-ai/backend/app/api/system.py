...
async def get_agent_guide() -> dict:
    ...
    return {
        ...
        "recommended_entrypoint": {
            ...
            "json_schema_hint": {
                "read_tools": ["query_papers", "get_paper", "get_codex_context", "get_codex_item", "get_paper_knowledge", "search_external_papers", "get_dft_review_queue", "get_dft_review_task", "get_dft_audit_issues", "get_correction_queue", "retrieve_evidence", "compare_papers", "read_paper_page", "review_figure", "get_review_coverage", "get_field_disputes", "scan_duplicate_dois"],
        ...
        "legacy_suggested_client_prompt": (
            ...
            "For local IDE AI, the primary DFT workflow is get_dft_review_task -> get_codex_item/read_paper_page -> acquire dft_results lock -> import_analysis(auto_apply_review_rules=true) -> readback. The offline DFT review ZIP is only for web AI, third-party, or offline review. DFT uses one controlled review-and-write workflow. After acquiring the dft_results module lock, one evidence-backed AI opinion may PASS, REVISE, REJECT, or create a result through import_analysis(auto_apply_review_rules=true). NEEDS_HUMAN remains pending. AI names, source labels, and identities are audit metadata, not admission votes. "
