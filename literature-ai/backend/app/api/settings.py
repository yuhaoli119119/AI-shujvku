...
async def get_ide_prompts(request: Request) -> dict[str, Any]:
    ...
    legacy_review_prompt = (
        ...
        "- Do not only write an audit report. For evidence-backed non-DFT content, write fixes back with import_analysis(auto_apply_review_rules=true) under a valid module write lock. If the payload also writes review_notes, relationships, or multiple non-DFT modules, prefer all_non_dft over a narrow module lock.\n"
    ...
    return {
        ...
        "legacy_suggested_prompt": (
            ...
            f"- import_analysis：除 DFT 最终确认外，按证据将 AI 审核结果写回；若 payload 同时写 notes、relationships 或多个非 DFT 模块，优先申请 all_non_dft 锁\n"
