...
class CodexContextService:
    ...
    def _build_item_markdown(self, context: dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "- Directly applying non-DFT text/object fixes uses import_analysis(auto_apply_review_rules=true); every formal section, writing_card, and mechanism_claim needs an object-level review bound to its real UUID plus exact PDF page evidence. [AI_REVIEWED] notes are explanatory only and never unlock RAG, writing, or citation. Non-DFT AI writes are last-writer-wins and do not require module write locks.",
