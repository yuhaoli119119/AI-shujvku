...
class CodexContextService:
        ...
        context: dict[str, Any] = {
            ...
            "review_scope": {
                ...
                "figure_derived_dft_policy": (
                    ...
                    "and material_identity when available. They must pass the dedicated ai_verify_content verification "
                    "and export safety gates before ML export."
                ),
            },
            "paper": {
                "id": str(detail.id),
    ...
    def _build_item_markdown(self, context: dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "- Write non-table conclusions back through import_analysis as structured review/correction candidates. Table object mutations must use the direct table tools. After any write, read back the target item or review coverage before claiming success.",
            ...
            "- For missing DFT rows, use object_review_audits with decision=new_candidate and a structured corrected_value. With a complete evidence anchor, import_analysis(auto_apply_review_rules=true) may materialize an unverified DFTResult candidate; it does not settle or export the row.",
            "- AI product names and source identities are audit metadata, not admission roles. Ordinary PASS, REVISE, REJECT, and new_candidate imports remain opinions/candidates. Authoritative AI acceptance uses one dedicated ai_verify_content identity.",
            "- The verifier reads get_ai_verification_tasks and submits accept, correct, reject, or exception through submit_ai_verification_batch. ai_verified remains distinct from human verified; only exception items enter Owner-session human handling.",
            ...
            "- An identity with repair_dft_issues may repair one audit issue into candidate data, but repair does not grant ai_verified, human verified, export, or ML readiness. False-positive closure remains human-only.",
            "- Treat the DFT audit center as a read-only queue and navigation entry. Use import_analysis for candidate opinions, then the dedicated ai_verify_content tools for authoritative AI acceptance; read back the row and export safety before reporting completion.",
            "- Ordinary non-DFT import_analysis output is retained under authenticated_human_review_required/no_ai_overwrite. For untrusted direct propose_correction writes, the server requires a module lock only for top-level abstract and structured sections, mechanism_claims, and writing_cards; other allowed metadata fields such as title/year/journal/authors are not universally lock-enforced. Tables and figures follow their dedicated tool capability/evidence contracts. [AI_REVIEWED] notes are explanatory only and never unlock RAG, writing, or citation.",
            "- Table object lifecycle is direct-tool-only: use update_table for caption/markdown/page/prov fixes, create_table for missing tables, merge_table for split/continued/duplicate table fragments, and delete_table for invalid table objects. Never submit table correction_proposals through import_analysis. Table object_review_audits may record PASS/REJECT opinions only; corrected_value is not applied.",
