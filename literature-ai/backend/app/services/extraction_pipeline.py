    ...
    def _normalized_dft_candidate_key(self, item: dict[str, Any]) -> str:
        ...
        return "|".join(
            [
                ...
                clean(item.get("active_site_instance_key")),
                clean(item.get("property_subtype")),
                clean(norm.get("adsorbate") or item.get("adsorbate")),
                clean(norm.get("property_type") or item.get("category")),
                clean(item.get("atom_pair")),
                clean(item.get("site_label")),
                clean(item.get("state_context")),
                value_key,
                clean(item.get("unit")),
                clean(item.get("reaction_step")),
                clean(item.get("source_table_id")),
                clean(item.get("source_row_index")),
                clean(item.get("source_column_index")),
            ]
        )
