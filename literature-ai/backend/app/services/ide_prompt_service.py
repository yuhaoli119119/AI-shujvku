...
PROMPT_SCHEMA_VERSION: Final = "ide_review_prompt_v3"
...
每篇文献的前置门：
- 先读取 get_codex_context，确认 paper_id、library_name、source_assets、artifact_status、pdf_quality_status 和 parse_allowed。
- 只有 A_text_readable 或 B_text_partial 且 parse_allowed=true 才继续正文核验。
...
- 每次写入后必须通过 MCP/API 回读对象、审核状态和审计记录。
