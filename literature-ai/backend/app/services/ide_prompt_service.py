...
- 主修复 AI 处理 DFT audit issue 时先调用 get_dft_audit_issues；只有具备 repair_dft_issues capability 时才可对单个 issue_id 调用 repair_dft_audit_issue，不做批量自动推进。审核 AI、普通 IDE AI、propose-only key 只能创建 issue/候选或读取队列，不能 repair。用户不需要复制审核 AI 长文本，给 issue_id 或让主 AI 读队列即可。
