...
def test_ide_prompts_always_require_http_mcp_key(monkeypatch):
    ...
    assert "Ordinary DFT PASS/REVISE/REJECT imports are opinions" in payload["legacy_english_suggested_prompt"]
    assert "decision=new_candidate may materialize an unverified DFTResult candidate" in payload["legacy_english_suggested_prompt"]
    assert "one ai_verify_content identity" in payload["legacy_english_suggested_prompt"]
    assert "get_ai_verification_tasks and submit_ai_verification_batch" in payload["legacy_english_suggested_prompt"]
    assert "only exceptions go to the Owner session" in payload["legacy_english_suggested_prompt"]
    assert "No second model, vote, consensus, or third-AI adjudication is used" in payload["legacy_english_suggested_prompt"]
    ...
    assert "普通 DFT PASS/REVISE/REJECT 通过 import_analysis 导入后仍是审核意见" in payload["legacy_suggested_prompt"]
    assert "new_candidate 只物化未验证候选" in payload["legacy_suggested_prompt"]
    assert "专用 ai_verify_content 身份" in payload["legacy_suggested_prompt"]
    assert "get_ai_verification_tasks 和 submit_ai_verification_batch" in payload["legacy_suggested_prompt"]
    assert "exception 才进入 Owner-session 人工处理" in payload["legacy_suggested_prompt"]
    assert "不使用第二模型、投票、共识或第三 AI 仲裁" in payload["legacy_suggested_prompt"]
