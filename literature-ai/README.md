...
**G3B scope note:** PDF evidence jump UI foundation is closed for current scope. The implementation covers iframe PDF preview, page jump, bbox/evidence side panel, and PDF unavailable fallback. Exact locator bbox coordinates and evidence_text are surfaced in the side panel — browser built-in iframe PDF viewer cannot reliably support coordinate-based pixel overlay, so no fake bbox highlight is rendered. Pixel-level PDF in-page highlighting is deferred to G3C (PDF.js Native Evidence Highlight).

### Recent Changes

| Date | Description | Files Affected |
|------|-------------|---------------|
| 2026-05-26 | **G3B PDF Evidence Jump UI — closed for current scope.** Implemented iframe PDF preview with page jump, bbox/evidence side panel, and PDF unavailable fallback. Backend now serves `GET /api/papers/{id}/pdf` via FileResponse. Fixed `escAttr()` for HTML attribute escaping (the original `esc()` via textContent→innerHTML does not escape `"`, which truncated `data-bbox` attributes containing JSON). Added HEAD probe before iframe load; PDF-missing papers show "PDF 尚未上传或不可预览" instead of an empty modal. Locator status degradation (exact/page_only/text_only/needs_reparse/missing) wired to evidence panel. No fake bbox overlay — browser built-in iframe PDF viewer cannot reliably support coordinate-based pixel overlay. Exact locator bbox coordinates and evidence_text are surfaced in the side panel for current scope. Verified by current Playwright smoke tests (66/66, 0 skipped). **Known limitation: pixel-level PDF in-page highlighting is not implemented; deferred to G3C PDF.js Native Evidence Highlight.** | `backend/app/api/papers/detail.py`, `frontend/pages/literature_library/render-detail.js`, `frontend/pages/literature_library/api.js`, `frontend/pages/literature_library/index.html`, `frontend/pages/external_analysis_workbench/index.html`, `frontend/tests/smoke.spec.js`, `README.md` |
| 2026-05-26 | G3A-F Evidence Locator Backend Hardening: backend locator safety patch hardened Stage2 reprocess cleanup for stale extraction locators, degraded malformed bbox metadata to page-only locators without fabricating coordinates, and returned 404 for missing claim locator lookups. Hardened for current scope and verified by current backend tests. | `backend/app/services/extraction_pipeline.py`, `backend/app/services/evidence_locator_service.py`, `backend/app/api/evidence.py`, `backend/tests/test_evidence_api.py`, `README.md` |
| 2026-05-25 | G3A PDF Provenance & Evidence Locator Backend: added a backward-compatible evidence locator model and APIs, threaded page/bbox/section/source metadata into claim and extraction evidence payloads, returned safe `page_only`/`text_only`/`missing`/`needs_reparse` fallbacks instead of fabricating coordinates, surfaced locator warnings during extraction validation, and preserved G1/G2 identity/review safety. Known limitation: this backend locator foundation is closed for current scope, but exact bounding boxes still depend on parser provenance that is not yet available for every legacy paper and extraction path. Verified by current backend tests. | `backend/app/db/models.py`, `backend/app/db/session.py`, `backend/app/schemas/evidence.py`, `backend/app/schemas/extraction.py`, `backend/app/services/evidence_locator_service.py`, `backend/app/services/evidence_service.py`, `backend/app/services/extraction_schema_service.py`, `backend/app/services/extraction_pipeline.py`, `backend/app/services/paper_ingestion.py`, `backend/app/api/evidence.py`, `backend/app/api/extraction.py`, `backend/app/api/papers/detail.py`, `backend/tests/test_evidence_api.py`, `backend/tests/test_extraction_reviews_api.py`, `backend/tests/test_papers_api.py`, `README.md` |
| 2026-05-25 | G2B Review Target Status UI: integrated review target resolution stability summary, HSL status override badges, click-to-expand warning boxes, three stability filters, saving/verification confirmations, and literature library paper detail alerts with full Playwright smoke coverage. | `frontend/pages/external_analysis_workbench/index.html`, `frontend/pages/literature_library/render-detail.js`, `frontend/pages/literature_library/api.js`, `frontend/tests/smoke.spec.js` |
| 2026-05-25 | G2 Review Target Stability Guard: added stable review target fingerprints, remapped extraction reviews after Stage2 re-extraction when semantic targets remained unchanged, marked stale/ambiguous reviews instead of silently applying them, preserved verified reviewer decisions, and exposed review resolution audit/status metadata for backend consumers. Known limitation: semantic remap fallback still depends on current label/evidence similarity heuristics and does not yet use page-span-aware provenance scoring. | `backend/app/db/models.py`, `backend/app/db/session.py`, `backend/app/services/review_target_resolver.py`, `backend/app/services/extraction_review_service.py`, `backend/app/services/extraction_schema_service.py`, `backend/app/services/extraction_pipeline.py`, `backend/app/api/extraction.py`, `backend/app/schemas/extraction.py`, `backend/tests/test_extraction_reviews_api.py`, `README.md` |
| 2026-05-25 | G1B-F stabilized frontend smoke suite after attach identity confirmation UI: mocked external Google Fonts CDN requests and added robust Playwright goto domcontentloaded configuration to eliminate flaky ERR_CONNECTION_TIMED_OUT errors under restricted network environments. | `frontend/tests/smoke.spec.js`, `README.md` |
...
1. **G3C PDF.js Native Evidence Highlight** — replace browser iframe PDF viewer with PDF.js renderer to support pixel-level bbox highlight overlay on PDF pages. G3B implemented evidence jump UI foundation (iframe preview + page jump + side panel), but browser built-in iframe cannot support coordinate-based pixel overlay. G3C will integrate PDF.js for native rendering and exact bbox highlighting.
