...
第一优先级是把检索到的文献稳定转化为可信 DFT 数据库，并能导出给机器学习使用。系统必须保留证据链、确认状态和原文出处，不能把未经确认的候选当作可训练事实。

第二优先级是把可信文献证据接入论文写作流程，先做“候选引用建议”和“引用草稿”，后续再对接 Zotero 等文献管理系统。写作侧只能辅助定位与草拟，不能自动生成最终引用、不能自动确认引用。

## 当前基线判断

项目已经具备文献库、解析任务、结构化提取、确认 review、安全导出门、证据定位、AI 写作、写作引用辅助等基础能力。下一阶段不应扩成泛用 RAG 平台，而应围绕“可信结构化科学数据闭环”继续加固。
