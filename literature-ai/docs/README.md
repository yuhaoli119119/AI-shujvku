...
- 已完成的后端/前端拆分包括 `review_conflict_service.py` 的 DFT helper 拆分、`paper_workbench_service.py` 的 review-center helper 拆分、workspace/source-document/audit/figure helper 拆分，以及 AI reading package/content coverage/DFT evidence payload helper 拆分；当前仍应把大型 orchestration service 视为后续小步拆分对象。
